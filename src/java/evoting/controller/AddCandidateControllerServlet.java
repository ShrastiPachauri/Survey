...
public class AddCandidateControllerServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(candidate!=null && candidate.equals("getid"))  //from adminoptions.js ,addnewcandidateForm
        {
            try
            {
                String id=CandidateDAO. getNewCandidateId();
                out.println(id); //response ,to be received in responseText ,b/c js is in between (ajax)
                return;
            }
        ...
        else if(usid!=null)//to fetch details of candidate from db ,when we enter userid aka aadhar no
        {
            
            try{
                
                String username=CandidateDAO.getUsernameById(usid);
                ArrayList<String> city=CandidateDAO.getCity();          //now we have to send these back to ajax
                ...
                out.println(json);//return as response 
            }
            catch(Exception ex)
            {
                RequestDispatcher rd=request.getRequestDispatcher("showexception.jsp");
