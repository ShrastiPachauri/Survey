...
public class VoterDAO {
    ...
    static {
        try {
            ...
            ps2 = DBConnection.getConnection().prepareStatement("select candidate_id, cname,party,symbol from candidate , user_details where candidate.userid = user_details.aadhar_no and candidate.city=(select city from user_details where aadhar_no=?)");
            ...
            ps7 = DBConnection.getConnection().prepareStatement("select user_details.gen, count(*) from user_details, voting where user_details.aadhar_no=voting.voter_id group by gen");
